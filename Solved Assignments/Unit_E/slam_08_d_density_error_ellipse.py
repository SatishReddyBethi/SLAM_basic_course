...
if __name__ == '__main__':
    ...
    f = open("Generated_files/particle_filter_ellipse.txt", "w")
    for i in range(len(logfile.motor_ticks)):
        ...
        mean = pf.get_mean()
        f.write(f"F {mean[0] + scanner_displacement * cos(mean[2]):.0f} {mean[1] + scanner_displacement * sin(mean[2]):.0f} {mean[2]:%.3f}\n")

        # Output error ellipse and standard deviation of heading.
        errors = pf.get_error_ellipse_and_heading_variance(mean)
        f.write(f"E {errors[0]:.3f} {errors[1]:.0f} {errors[2]:.0f} {errors[3]:.3f}\n")
